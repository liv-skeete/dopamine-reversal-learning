#!/usr/bin/env python3
"""
Hedonic Experiment Runner
=========================

Main script to run hedonic experiments from the original Colab code.
"""
...
def run_session(agent, track_meta=False):
    """Run a single session of the hedonic experiment."""
...
def main():
    """Run the main hedonic experiment."""
    print("Running Hedonic Experiment...")
    ...
    # Run experiments
    normal_df, meta_normal = run_group("Normal", 1.0, 10, track_meta=True)
    addicted_df, _ = run_group("Addicted", 1.0, 12, track_meta=False)
    ...
    for i in range(10):
        _, logs = run_group("Normal", 1.0, 1, track_meta=True)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("../results/hedonic_meta_strategy.png", dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.suptitle("Hedonic Reward Theory — Hybrid (V→Q) vs. Addicted (Q-only)", fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig("../results/hedonic_errors.png", dpi=300, bbox_inches='tight')
    plt.show()
    
    # Save results
    df.to_csv("../results/hedonic_results.csv", index=False)

#!/usr/bin/env python3
"""
Prediction Error Experiment Runner
==================================

Main script to run prediction error experiments from the original Colab code.
"""
...
def run_session(agent, track_meta=False):
    """Run a single session of the prediction error experiment."""
    ...
    for trial in range(TRIALS_PER_SESSION):
        ...
        # Off-policy accuracy calculations
        v_vals = []
...
def run_group(name, bias, n=10, track_meta=False):
    ...
    for agent_idx in range(n):
        agent = PredictionErrorAgent(name, bias)
        ...
        for session_idx in range(SESSIONS):
            agent.reset_session(session_idx)
...
def main():
    """Run the main prediction error experiment."""
    print("Running Prediction Error Experiment...")
    ...
    # Run experiments
    normal_df, meta_normal = run_group("Normal", 0.0, 10, track_meta=True)
    addicted_df, _ = run_group("Addicted", 0.6, 12, track_meta=False)
    df = pd.concat([normal_df, addicted_df])
    
    # Create meta-strategy plot
    meta_matrix = np.zeros((10, TRIALS_PER_SESSION))
    q_acc_matrix = np.full((10, TRIALS_PER_SESSION), np.nan)
    v_acc_matrix = np.full((10, TRIALS_PER_SESSION), np.nan)
    v_off_acc_matrix = np.zeros((10, TRIALS_PER_SESSION))
    q_off_acc_matrix = np.zeros((10, TRIALS_PER_SESSION))
    
    for i in range(10):
        _, logs = run_group("Normal", 0.0, 1, track_meta=True)
        meta_matrix[i, :] = logs['meta'][:TRIALS_PER_SESSION]
        q_acc_matrix[i, :] = logs['q_acc'][:TRIALS_PER_SESSION]
        v_acc_matrix[i, :] = logs['v_acc'][:TRIALS_PER_SESSION]
    ...
    q_mean = pd.Series(np.nanmean(q_acc_matrix, axis=0)).fillna(0).rolling(window, min_periods=1).mean()
    v_mean = pd.Series(np.nanmean(v_acc_matrix, axis=0)).fillna(0).rolling(window, min_periods=1).mean()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("../results/prediction_error_meta_strategy.png", dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.suptitle("Prediction Error Theory — Hybrid (V→Q) vs. Addicted (Q-only)", fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig("../results/prediction_error_errors.png", dpi=300, bbox_inches='tight')
    plt.show()
    
    # Save results
    df.to_csv("../results/prediction_error_results.csv", index=False)
